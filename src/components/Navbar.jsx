    ...
    <AnimatePresence>
      {showNav && (
        <motion.nav
          ...
          className="fixed top-0 left-0 w-full z-50 text-white px-6 md:px-20 py-6 md:py-8 bg-zinc-900 backdrop-blur-sm shadow-lg"
        >
            ...
            <div className="hidden md:flex gap-10 items-center">
              {navItems.map((item, index) => (
                <a
                  key={index}
                  href="#"
                  className={`text-sm capitalize font-light hover:text-zinc-400 cursor-pointer ${
                    index === navItems.length - 1
                      ? "ml-16 px-6 py-2 bg-zinc-700 rounded-full shadow-2xl hover:scale-105 transition-transform"
                      : ""
                  }`}
                >
                  {item}
          ...
          {/* Mobile Menu */}
          {menuOpen && (
            <div className="md:hidden mt-6 bg-zinc-800 rounded-xl py-6 flex flex-col gap-6 items-center">
              {navItems.map((item, index) => (
                <a
                  key={index}
                  href="#"
                  className={`text-md capitalize font-light text-white hover:text-zinc-400 ${
                    index === navItems.length - 1
                      ? "px-6 py-2 bg-zinc-700 rounded-full shadow-2xl hover:scale-105 transition-transform"
                      : ""
                  }`}
